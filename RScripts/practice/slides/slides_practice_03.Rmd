...
.red[
```{r, echo = F}
message("Ошибка в is_temp_celsius_hot(c(12, 18, 42)) :объект 'output' не найден\n Вдобавок: Предупреждение:\n В if (temp_C > 35) output <- TRUE :\n  длина условия > 1, будет использован только первый элемент")
```
]]
...
.green[
**Пользуемся векторизацией операторов `<` `>` `==` `!=`**
]
...
```{r, eval = F}
temp_to_celsius_bad <- function(temp) {
...   # теперь в функции нет аргумента temp_scale
}

```

```{r}
temp_scale <- 'C'  # это глобальная temp_scale
temp_to_celsius_bad(df_weather$temp)

```

```{r, eval = F}
rm(temp_scale)     # глобальной переменной temp_scale нет
temp_to_celsius_bad(df_weather$temp)
```

.red[
```{r, echo = F}
message("Ошибка в temp_to_celsius_bad(df_weather$temp) : \nобъект 'temp_scale' не найден")
```
]
